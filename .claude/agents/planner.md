...
- `docs/tech-stack.md` — the concrete toolchain and the application file layout (locked in bootstrap Phase 5). Until it exists: TypeScript strict, boundary validation with a schema library, tests via the runner it names. **BLOCK if the brief pins a tool `docs/tech-stack.md` does not name.**
...
- Q5 (genuine owner decision): STOP (rare). Changing a locked decision D1–D19 is **always** Q5 — cite the D-number.

# BLOCK protocol
BLOCK when: the brief file is missing; the brief contradicts a contract or an invariant I1–I13; the brief pins a tool `docs/tech-stack.md` does not name; or a requested task id has no resolvable spec path. On BLOCK, return one JSON object with `error` populated (cite the contradicting passages), `tasks: []`. Do not retry, do not infer, do not write files.
