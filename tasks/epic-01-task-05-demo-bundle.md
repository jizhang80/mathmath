...
- Any file under `Packages/Core/**` or `pipeline/**` — this task ships no code, only data. If `core-cli validate` or the pipeline wrapper do not yet exist/work as task 01.4 specifies, that is a blocking dependency gap, not something this task patches.
...
10. Smoke check: from the repo root, `pytest pipeline/tests/test_contracts.py -k "data_bundles or verbatim"` — must be green before proceeding to the full gate.
...
**Companion test this task ships** (per "Companion tests belong to the task that ships the code"): add `pipeline/tests/test_demo_bundle.py` (or extend `test_contracts.py` if the implementer judges that more consistent with existing pipeline test layout — the file name is the implementer's call, not fixed by this spec) asserting, over `data/demo/` specifically and scoped to `data/demo/`'s own files (not a whole-repo scan, so a later task's `data/<other-bundle>/` cannot falsify it):
...
- `MTH1W`'s `next_courses == ["MPM2D"]` and `MCR3U`'s `next_courses == ["MHF4U"]`;
...
- IF a course's `next_courses[]` target (`MPM2D`, `MHF4U`) does not exist as a `Course` entry in `courses.json` THEN leave it as a bare string in `next_courses[]` (the schema only requires it to match the course-code pattern, not to resolve to a sibling entry in this file) — per `contracts/schemas/courses.schema.json`'s `next_courses` item type (`string`, pattern only, no `$ref` to another course) quoted in full in §3's schema citation.
...
- format + lint clean (`ruff check` / `ruff format --check` over `pipeline/` if the companion test file changes Python; no Swift files are touched by this task so `swift-format lint --strict` is unaffected)
...
- `pytest pipeline/tests/test_demo_bundle.py` (or wherever the companion test lands) green, covering every bullet in §5's companion-test list
