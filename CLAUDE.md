...
- The stack is locked in **bootstrap Phase 5** — **do not pin versions until `docs/tech-stack.md` exists**; BLOCK on any spec pinning a tool that file does not name. No third-party dependency beyond SwiftMath without a recorded reason (D32). No third-party game engine (D24); Apple's first-party SpriteKit is permitted only if `Canvas` performance demands it.
- Gates until the stack is locked: `xcodebuild` build + test on the iOS simulator for Swift; typecheck + lint + test for Python, per `docs/tech-stack.md`. **Physical-device verification is the owner's delivery verification at the wrap-gate; no agent task may claim it** (D29).
