...
**mathmath** (working name; candidate *Upstream*; never `mathpath`) — a math learning system for students whose continent runs from foundational through undergraduate engineering mathematics (D43); Ontario grades 9–12 is the first content tier, not the product boundary (D1). One cross-grade concept graph is rendered as a **map** organised by math's own taxonomy (Door C); students move across it in ~3-minute **expeditions** of probe items that lift fog (Door B); when a node is blocked, an in-map **diagnosis** event walks upstream to the deepest unmastered prerequisite, confirms it with a ~60-second probe, remediates the minimum piece and returns (Door A). **One map, one trail per student** (D47): the trail is generated from the selected course syllabi, a course-progress marker the student sets from the course's unit list (D45) and their mastery state; new-learning items come only from the trail's outer fringe (D48). Landmarks are real, sourced, real-world things linked to nodes. It is a **single-user native iOS/iPadOS app in Swift** — no accounts, no parent view; Android is a later port. Runtime infrastructure is static hosting of versioned content JSON plus one anonymous telemetry write endpoint; **no application server**. The desktop web homework mode (structured editor + CAS step verification) is deferred to M5.

**Ground truth: [`PROJECT-BRIEF-v2.md`](PROJECT-BRIEF-v2.md) as amended by [`AMENDMENT-v2.1.md`](AMENDMENT-v2.1.md) … [`AMENDMENT-v2.7.md`](AMENDMENT-v2.7.md)** — a later amendment wins on conflict; amendments are deltas, the brief is not rewritten. [`docs/idea.md`](docs/idea.md) is the consolidated extract (decisions in force, milestones, unknowns). Decisions **D1–D49 are locked** (D30 and D37 are unassigned; never reuse them; D28 is superseded by D45). Changing a D-number is **always a Q5 owner decision** — never an agent's call.
...
| I6 | **No verbatim Ministry text** stored or shipped: grade 9–12 nodes carry expectation codes + the project's own `paraphrase`, and link out to the official page; undergraduate nodes carry a `source_ref` into a CC-licensed source with its attribution. | D18, D2 |
| I7 | **One cross-grade graph**; a course is a node subset + depth marker; a student's trail is generated over the graph, never authored per course. Never 11 separate syllabi. | D3, D20, D47 |
| I8 | Every accepted graph passes the **L0 checks**: acyclic; no later→earlier course edge; code↔node coverage both ways **for nodes carrying `expectation_codes` and for every Ministry expectation**; a resolvable `source_ref` on every node without codes (a node with neither fails); in-degree outliers flagged; starting chain connected; **every node has exactly one region; every generated trail is well-formed over the graph** — a course segment is exactly that course's resident nodes in unit order, topological within a unit (an edge against unit order is reported, not failed); every extension node is reachable from the segment before it. | §5, v2.7 §1, Q5 02-QE |
